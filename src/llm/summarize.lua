...
local log = logger:named("keeper.llm.summarize")
...
local function elapsed_ms(started_ns)
    ...
    if not ok or type(now) ~= "number" or type(started_ns) ~= "number" then
        return nil
    end
    return math.floor((now - started_ns) / 1e6)
end
