...
local function define_tests()
    describe("MCP", function()
        ...
        after_all(function()
            ...
            for _, raw in ipairs(created_tokens) do
        ...
        local function track(tok)
            if tok and tok.token then table.insert(created_tokens, tok.token) end
            return tok
        end
        ...
        describe("tokens", function()
            ...
            it("persists trait_filter and tool_filter as JSON", function()
                local trait_filter = { tags_any = { "state", "knowledge" }, namespaces = { "keeper.traits" } }
                ...
                test.eq(fetched.trait_filter.namespaces[1], "keeper.traits")
            ...
            it("raw bearer is never stored at rest (sha256 hash only)", function()
                local tok = track(mcp_tokens.create({
                    label = "hash-at-rest-" .. uuid.v4(),
                    scopes = { "registry.read" },
                }))
                test.not_nil(tok)
                test.not_nil(tok.token)

                local db = sql.get(mcp_consts.DB_ID)
                local rows = db:query(
        ...
        describe("filter_matches", function()
            ...
            it("non-table filter is treated as nil", function()
                test.is_true(fm("not-a-table", "any.ns:entry", {}))
            end)
            ...
            it("include_ids unconditionally passes", function()
                local f = { include_ids = { "ns:keep" }, exclude_ids = { "ns:keep" } }
                test.is_true(fm(f, "ns:keep", {}))
            end)
            ...
            it("namespaces restricts by id prefix before colon", function()
                local f = { namespaces = { "keeper.traits", "userspace.traits" } }
                test.is_true(fm(f, "keeper.traits:editor", {}))
                test.is_true(fm(f, "userspace.traits:foo", {}))
            ...
            it("namespace filter matches sub-namespaces", function()
                ...
                test.is_true(not fm(f, "keeper.agents.traits.debug:flow_debugger", {}))
            end)
        ...
        describe("traits catalog", function()
            ...
            it("trait_allowed rejects unknown id", function()
                local allowed, err = mcp_traits.trait_allowed(
                    { trait_filter = { namespaces = { "keeper.traits" } } },
                    "keeper.traits:does_not_exist_xyz"
                )
        ...
        describe("traits active-set lifecycle", function()
            ...
            it("set_active replaces the set and validates ids", function()
                ...
                local bad, berr = mcp_traits.set_active(session, { "keeper.traits:nope_nope" })
                test.is_nil(bad)
                test.not_nil(berr)
            end)
        ...
        describe("resolve_tools_only", function()
            ...
            it("nil tool_filter returns every registered tool", function()
                local session = {
                    label = "test",
                    identity = "test",
                    access_mode = "tools_only",
        ...
        describe("prune_missing_traits", function()
            ...
            it("keeps real traits, drops unknowns", function()
                local kept, dropped = prune({
                    ...
                    "keeper.traits:does_not_exist_abc",
            ...
            it("rejects non-trait registry entries", function()
                ...
                local kept, dropped = prune({ "keeper.state.tools:explore" })
                test.eq(#kept, 0)
                test.eq(#dropped, 1)
            end)
        end)
        ...
        describe("resolve self-heal", function()
            it("strips stale trait ids from persisted active set", function()
                ...
                mcp_tokens.set_active_traits(session.token, {
                    "keeper.agents.traits.state:explorer",
                    "keeper.traits:removed_offline_xyz",
                })
            ...
            it("returns no trait-derived tools when every active id is stale", function()
                ...
                local session = mcp_tokens.get(tok.token)
                mcp_tokens.set_active_traits(session.token, {
                    "keeper.traits:gone_1",
                    "keeper.traits:gone_2",
                })
        ...
        describe("tool_allowed guard", function()
            ...
            it("rejects unknown tool id", function()
                local allowed, err = mcp_traits.tool_allowed(
                    { tool_filter = nil },
                    "keeper.tools:does_not_exist_xyz"
                )
                test.is_true(not allowed)
                test.not_nil(err)
            end)
        ...
        describe("meta-tool ergonomics", function()
            local function new_session(opts)
                ...
                local tok = track(mcp_tokens.create({
                    ...
                    identity = "root",
                    scopes = opts.scopes or { "registry.read" },
