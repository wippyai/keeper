...
function tokens.create(params)
    ...
    local raw_token, uerr = uuid.v4()
    if uerr then return nil, "Failed to generate token: " .. tostring(uerr) end
    ...
    local _, exec_err = db:execute([[
        INSERT INTO keeper_mcp_tokens
          (token, label, identity, scopes, access_mode, available_traits, available_tools, default_active, created_at, expires_at, revoked)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
    ]], {
        token_hash,
        params.label or "",
...
function tokens.get(raw_token)
    ...
    local rows, query_err = db:query([[
        SELECT token, label, identity, scopes, access_mode, available_traits, available_tools,
               default_active, created_at, expires_at, revoked
...
-- their hash.
function tokens.revoke(token_hash)
    ...
    local _, exec_err = db:execute(
        "UPDATE keeper_mcp_tokens SET revoked = 1 WHERE token = ?",
        { token_hash }
    )
    db:release()
