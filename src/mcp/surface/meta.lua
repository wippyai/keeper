...
function M.describe_trait(args, session)
    if not args or not args.id then return nil, "id required" end
    local def, err = mcp_traits.describe(args.id, session)
    ...
    if not name or name == "" or not description or description == "" then
        local entry = registry.get(args.id)
