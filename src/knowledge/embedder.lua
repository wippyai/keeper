...
local function embed(args: {node_id: string?, model: string?}?)
    if type(args) == "string" then args = {} end
    args = args or {}
    local tbl_args = (args :: any)
    if tbl_args.node_id then
        return embed_node(tbl_args.node_id, { model = tbl_args.model })
    end
    ...
    for _, row in ipairs(rows or {}) do
        local result, err = embed_node(row.id, { model = tbl_args.model })
        if result then
            count = count + 1
            pcall(function()
