...
local function handler()
    ...
    local result, err = kb_service.learn({
        kb = body.kb,
    ...
    if result.dataflow_id == nil then
        res:set_status(http.STATUS.OK)
        res:set_content_type(http.CONTENT.JSON)
        res:write_json({ success = true, message = result.message or "No research topics generated",
            topics = 0,
    ...
    res:set_status(http.STATUS.OK)
    res:set_content_type(http.CONTENT.JSON)
    res:write_json({ success = true, dataflow_id = result.dataflow_id,
