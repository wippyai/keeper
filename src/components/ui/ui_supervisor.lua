...
local function project_root()
    return env.get(consts.PROJECT_ROOT_ENV) or ""
end
...
local function build_storage_state(origin, token)
    ...
    local stored = json.encode({
        token = token,
        expiresAt = os.date("!%Y-%m-%dT%H:%M:%S.000Z", os.time() + 24 * 60 * 60),
    })
    return {
        cookies = {},
        origins = {
...
local function close_streams(state)
    local s = state :: any
    if s.stdout_stream then
        pcall(function() (s.stdout_stream :: any):close() end)
    end
    if s.stderr_stream then
        pcall(function() (s.stderr_stream :: any):close() end)
    end
    s.stdout_stream = nil
    s.stderr_stream = nil
end
...
local function kill_container(state)
    ...
    local proc_any = state.proc :: any
    pcall(function() proc_any:write_stdin(json.encode({ id = -1, op = "quit" }) .. "\n") end)
    pcall(function() proc_any:write_stdin("") end)  -- EOF
    wait_with_watchdog(state)
    close_streams(state)
    if state.shell then
        local sh = state.shell :: any
        if sh.release then pcall(function() sh:release() end) end
    end
    state.proc = nil
    state.proc_name = nil
    state.shell = nil
...
local function send_one(state, op, args, timeout_s)
...
local function call_container(state, op, args, timeout_s)
    local fresh = false
...
local function handle_command(state, msg)
    ...
    local response, err = call_container(state, tostring(op), processed :: any, tonumber(timeout_s))
    if err then
