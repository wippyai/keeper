...
local function send_reply(from, payload, result, err)
    if not from or not payload or not payload.respond_to then
        return
    end
    if err then
...
local function handle_create(payload, from)
...
local function handle_open_or_resume(payload, from)
    local args = payload.args or {}
...
local function handle_lock(payload, from)
    local args = payload.args or {}
...
local function handle_unlock(payload, from)
    local args = payload.args or {}
...
local function handle_edit(payload, from)
    local args = payload.args or {}
...
local function handle_drop(payload, from)
    local args = payload.args or {}
...
local function handle_transition(payload, from)
    local args = payload.args or {}
...
local function handle_list_changes(payload, from)
    local args = payload.args or {}
...
local operation_handlers = {
    [consts.OPERATIONS.CREATE]         = handle_create,
    [consts.OPERATIONS.OPEN_OR_RESUME] = handle_open_or_resume,
    [consts.OPERATIONS.EDIT]           = handle_edit,
    [consts.OPERATIONS.DROP]           = handle_drop,
    [consts.OPERATIONS.TRANSITION]     = handle_transition,
    [consts.OPERATIONS.LIST_CHANGES]   = handle_list_changes,
    [consts.OPERATIONS.LOCK]           = handle_lock,
    [consts.OPERATIONS.UNLOCK]         = handle_unlock,
}
...
local function handle_message(msg)
    ...
    local handler = operation_handlers[payload.operation]
    if not handler then
        log:warn("unknown operation", { operation = payload.operation, from = from })
        return send_reply(from, payload, nil,
            "unknown operation: " .. tostring(payload.operation))
    end
...
local function run()
    ...
    while true do
        ...
        elseif result.channel == proc_events then
            -- Phase 1 has no child workers, so nothing to handle here.
            -- Phase 5 will re-spawn push/drift workers and track their exits.
