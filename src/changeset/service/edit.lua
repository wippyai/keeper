...
function M.assert_live(changeset)
...
function M.validate_args(args)
    if type(args) ~= "table" then
        return consts.ERRORS.MISSING_REQUIRED .. ": changeset_id"
    end
    if not args.changeset_id or args.changeset_id == "" then
        return consts.ERRORS.MISSING_REQUIRED .. ": changeset_id"
    end
    if not args.kind or args.kind == "" then
        return consts.ERRORS.MISSING_REQUIRED .. ": kind"
    end
    if not VALID_KINDS[args.kind] then
        return "unknown edit kind: " .. tostring(args.kind)
    end
    return nil
end

function M.compute_entry_hash(entry)
...
local function preserved(branch, entry_id, chunk_type)
    local txt, err = repo.read_chunk_text(branch, entry_id, chunk_type)
    if err then return nil, err end
    if txt ~= nil then return txt, nil end
    return repo.read_chunk_text(consts.MAIN_BRANCH, entry_id, chunk_type)
end

local function record_pending(changeset_id, category, op, target, current_hash)
...
local function run_state_op(changeset, command_type, payload, err_prefix)
...
local function run_registry_set(changeset, args)
...
local function run_registry_delete(changeset, args)
    if not args.entry_id then
    ...
    return { ok = true, entry_id = args.entry_id, result = result }, nil
end

-- Write a file to the changeset scratch FS.
local function run_fs_write(changeset, args)
    if not args.rel_path then
    ...
    local h, werr = view:write(args.rel_path, args.content)
    ...
    return { ok = true, rel_path = args.rel_path, current_hash = h }, nil
end

-- Mark a file deleted in the changeset.
local function run_fs_delete(changeset, args)
    if not args.rel_path then
        return nil, consts.ERRORS.MISSING_REQUIRED .. ": rel_path"
    end
    ...
    local _, derr = view:delete(args.rel_path)
    ...
    return { ok = true, rel_path = args.rel_path }, nil
end
