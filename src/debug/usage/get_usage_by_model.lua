...
local function handler()
    ...
    local model_data, err = time_utils.query_by_model(tr)
    if err then
        res:set_status(http.STATUS.INTERNAL_ERROR)
        res:set_content_type(http.CONTENT.JSON)
        res:write_json({ success = false, error = "Failed to get usage by model: " .. err })
        return
    end

    local totals = time_utils.compute_totals(model_data)
    time_utils.apply_percentages(model_data, totals.total_tokens)
