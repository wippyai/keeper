...
local function wait_for(ch, timeout)
...
local function run_one(executor, entry, failures, per_case, test_done_ch)
    ...
    if cmd_err then
        ...
        per_case[entry_id] = { passed = 0, failed = 1, skipped = 0 }
        return
    end

    local response = wait_for(cmd:response(), timeout)
    if not response then
        ...
        per_case[entry_id] = (per_case[entry_id] or { passed = 0, failed = 0, skipped = 0 })
        per_case[entry_id].failed = per_case[entry_id].failed + 1
        return
    end
    ...
    elseif payload and payload:data() == false then
        ...
        per_case[entry_id] = { passed = 1, failed = 0, skipped = 0 }
    end
end
...
local function do_handler(params)
    ...
    coroutine.spawn(function()
        while true do
            ...
            if not per_case[ref_id] then
                per_case[ref_id] = { passed = 0, failed = 0, skipped = 0 }
            end

            if mtype == "test:case:pass" then
                per_case[ref_id].passed = per_case[ref_id].passed + 1
            elseif mtype == "test:case:fail" then
                per_case[ref_id].failed = per_case[ref_id].failed + 1
            ...
            elseif mtype == "test:case:skip" then
                per_case[ref_id].skipped = per_case[ref_id].skipped + 1
            end
