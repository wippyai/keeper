...
local function render_tool_call(flow, node, index)
    ...
    local out = {}
    table.insert(out, string.format("# Tool call #%d of %d", index, #calls))
    if index > #calls then
        table.insert(out, "")
    ...
    table.insert(out, "name: `" .. (c.call.name or "?") .. "`")
    table.insert(out, "registry_id: `" .. (c.call.registry_id or "?") .. "`")
    if c.call.id then
        table.insert(out, "id: `" .. c.call.id .. "`")
    end
    table.insert(out, "")
    table.insert(out, "### Arguments")
    local args_s
    do
        local ok, enc = pcall(json.encode, c.call.arguments)
        args_s = ok and enc or tostring(c.call.arguments)
    end
    table.insert(out, "```json")
    ...
    for i = math.max(1, index - 2), math.min(#calls, index + 2) do
        ...
        table.insert(out, string.format("  %d. %s%s", i, n.call.name or "?", marker))
    end
    return table.concat(out, "\n")
end
...
function ACTIONS.diff(params)
    ...
    local function counts(nodes)
        local c = { completed = 0, failed = 0, running = 0, pending = 0 }
        for _, n in ipairs(nodes) do c[n.status] = (c[n.status] or 0) + 1 end
        return c
    end
