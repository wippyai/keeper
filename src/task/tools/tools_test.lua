    ...
    describe("Task tools", function()
        ...
        describe("save_context", function()
            ...
            it("persists a finding node with metadata", function()
                ...
                test.eq(rows[1].title, "api_pattern")
                test.eq(rows[1].content, "Auth via app:api router")
                test.eq(rows[1].type, "finding")
                test.eq(rows[1].discriminator, "api_pattern")
                test.eq(rows[1].status, "active")
                test.eq(rows[1].metadata.comment, "for later")
            end)
        end)
        ...
        describe("write_spec", function()
            ...
            it("creates a spec node and updates the task spec+phase", function()
                ...
                test.eq(#rows, 1)
                test.eq(rows[1].title, "Impl Spec")
                test.eq(rows[1].content, "Create keeper.foo:bar as function.lua")
                test.eq(rows[1].discriminator, "1")
                test.eq(rows[1].status, "active")
            ...
            it("defaults the node title to include revision number", function()
                ...
                local rows = nodes_reader.by_type(task_id, "spec")
                test.eq(#rows, 1)
                test.is_true(rows[1].title:find("Implementation Specification") ~= nil,
            ...
            it("increments revision on subsequent writes and supersedes prior", function()
                ...
                test.eq(rows[1].discriminator, "1")
                test.eq(rows[1].status, "superseded")
                test.eq(rows[2].discriminator, "2")
                test.eq(rows[2].status, "active")
                test.eq(rows[1].metadata.is_final, false)
            end)
        end)
