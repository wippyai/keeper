...
local function define_tests()
    describe("Phase state machine", function()
        ...
        describe("bounce_cap", function()
            it("returns review->implement cap=1 toward finish (regression: reviewer bounces once, then force-finish)", function()
                local cap = sm.bounce_cap(P.REVIEW, P.IMPLEMENT)
                test.not_nil(cap)
                test.eq(cap.cap, 1,
                    "review->implement cap must be 1; increasing lets buggy work loop indefinitely")
                test.eq(cap.terminal, P.FINISH)
            end)
            ...
            it("returns implement->review cap toward finish", function()
