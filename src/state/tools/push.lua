...
local function record_changes(rows, head_changeset_id, head_version)
...
local function registry_rows_from_ops(changeset_id, ops)
    local rows = {}
    for _, op in ipairs(ops) do
        local entry = op.entry or {}
        local registry_op = op.kind or REGISTRY_OPS.CREATE
        local category = CS_CATEGORIES.REGISTRY
        local source   = CS_SOURCES.OVERLAY_PUSHED
        local status   = CS_CHANGE_STATUSES.APPLIED
        table.insert(rows, {
            changeset_id = changeset_id,
            category     = category,
            op           = registry_op,
...
local function fs_rows_from_paths(changeset_id, written, deleted)
    local rows = {}
    for _, w in ipairs(written or {}) do
        table.insert(rows, {
            changeset_id = changeset_id,
            category     = CS_CATEGORIES.FILESYSTEM,
            op           = CS_OPS.WRITE,
            target       = w.path,
            source       = CS_SOURCES.FS_FLUSHED,
            status       = CS_CHANGE_STATUSES.APPLIED,
        })
    end
...
local function do_handler(params)
    ...
    local journal_rows = registry_rows_from_ops(changeset_id, changeset)
