...
local log = logger:named("state.agent.compare")
...
local function classify_changes(source_map, target_map)
    ...
    for id, target_entry in pairs(target_map) do
        ...
        elseif not source_entry or source_entry.deleted == 1 then
            ...
            local source_def, source_src = get_entry_content(source_entry)
            local target_def, target_src = get_entry_content(target_entry)

            local source_hash = compute_content_hash(source_def, source_src)
...
local function generate_unified_diff(old_text, new_text, context_lines)
...
local function tree_mode(params)
    ...
    for _, id in ipairs(changes.added) do
        ...
        if ns then
            ...
            namespace_stats[ns].added = namespace_stats[ns].added + 1
        end
    end
    ...
    for _, id in ipairs(changes.deleted) do
        ...
        if ns then
            ...
            namespace_stats[ns].deleted = namespace_stats[ns].deleted + 1
        end
    end
    ...
    for _, id in ipairs(changes.modified) do
        ...
        if ns then
            ...
            namespace_stats[ns].modified = namespace_stats[ns].modified + 1
        end
    end
...
local function handler(params)
    ...
    if params.mode == "tree" then
        return tree_mode(params)
    elseif params.mode == "summary" then
        return summary_mode(params)
    elseif params.mode == "entries" then
        return entries_mode(params)
    else
