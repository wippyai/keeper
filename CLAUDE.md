...
**Maintenance rule:** when one of the four blocks changes, update the other three. The 46 `@dark` keys + 29 `@light` keys + 16 top-level keys (under `cssVariables`) plus the `customCSS` string (peer of `cssVariables`, currently the JetBrains Mono `@import`) MUST stay identical across all four. The keeper-v5 `usage` analytics app deliberately omits `configOverrides` (it inherits from the host facade, per rev-3 §5.1) — do not add a fifth copy there.
