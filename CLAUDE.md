...
- `Proxy globals not found` at `entry.web-component.ts:93` (source line range 92–105) — host did not write `window.__WIPPY_APP_API__` before the WC bundle imported `@wippy-fe/proxy`. Usually means the harness is pinned to an older `web-host` version where the connector-mount fix isn't present; override `wippy/facade`'s `fe_facade_url` to a version ≥ 1.0.28 (e.g. `-o wippy.facade:fe_facade_url:default=https://web-host.wippy.ai/webcomponents-1.0.28`, or `http://localhost:5173` for local-dev gen-2-chat). 1.0.28 is the current supported floor as of 2026-05-12.
