...
-- The /llms.txt file at the docs root is the canonical, hand-curated index
-- of every doc page intended for LLM consumption. It is much shorter than
-- the full TOC and points researchers at the right starting page without
-- needing to crawl. Treated as a separate action because it lives at the
-- root, not under /llm.
local LLMS_INDEX_URL = "https://wippy.ai/llms.txt"
...
local function do_handler(params)
    ...
    elseif action == "index" then
        -- Fetch the curated llms.txt index. Cheaper + denser than `toc`
        -- when you just need to know "what docs exist for area X".
        local response, err = http_client.get(LLMS_INDEX_URL)
        if err then return nil, "llms.txt fetch failed: " .. err end
        if response.status_code ~= 200 then return nil, "llms.txt returned " .. response.status_code end

        local body = response.body
        if not body or body == "" then return "llms.txt unavailable." end

        return body

    else
        return nil, "Unknown action: " .. action .. ". Use: search, fetch, toc, index"
    end
end
