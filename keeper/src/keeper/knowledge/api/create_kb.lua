...
local function handler()
    ...
    if not body then
        ...
        res:set_content_type(http.CONTENT.JSON)
        res:write_json({ success = false, error = "Invalid request body" })
        return
    end

    local kb, err = kb_service.create_kb({ name = body.name, description = body.description })
    if err then
        local _err = err
    ...
    res:set_content_type(http.CONTENT.JSON)
    res:write_json({ success = true, kb = kb })
end
