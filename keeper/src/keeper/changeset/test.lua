...
local function define_tests()
    ...
    describe("fs_hash determinism", function()
        ...
        it("filesystem_diff compares local-module frontend paths against baseline", function()
            local ws, err = new_test_changeset({ title = "Plugin Frontend Diff" })
            test.is_nil(err)
            test.not_nil(ws)
            local target = "plugins/git/frontend/applications/git/package.json"

            local _, serr = repo.store_fs_content(ws.changeset_id, target, "{\"changed\":true}", "plugin-hash")
            test.is_nil(serr)

            local rows, derr = diff.filesystem_diff(ws.changeset_id)
            test.is_nil(derr)
            local found = nil
            for _, row in ipairs(rows or {}) do
                if row.target == target then found = row end
            end
            test.not_nil(found)
            test.eq(found.op, consts.OPS.UPDATE)
            test.eq(found.current_hash, "plugin-hash")
        end)
    end)
    ...
    describe("repo CRUD", function()
        ...
        it("records fs deletes and lists them", function()
            ...
            if not deletes_after then error("expected delete rows") end
            test.eq(#deletes_after, 1)
            local delete_after = deletes_after[1]
            if not delete_after then error("expected remaining delete row") end
            test.eq(delete_after.rel_path, "baz/qux.vue")
        end)
    ...
    describe("additional coverage", function()
        ...
        it("list_fs_content filters flushed by default", function()
            ...
            if not hist_only then error("expected flushed content rows") end
            test.eq(#hist_only, 1)
            local hist_content = hist_only[1]
            if not hist_content then error("expected flushed content row") end
            test.eq(hist_content.rel_path, "frontend/a.vue")
        end)
        ...
        it("list_fs_deletes filters flushed by default", function()
            ...
            if not hist then error("expected flushed delete rows") end
            test.eq(#hist, 1)
            local hist_delete = hist[1]
            if not hist_delete then error("expected flushed delete row") end
            test.eq(hist_delete.rel_path, "frontend/gone.vue")
        end)
