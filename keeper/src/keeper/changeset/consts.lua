local consts = {
    ...
    -- Used by keeper.changeset.service:edit_dispatch dispatch and keeper.state.patch:engine.
    EDIT_KINDS = {
    ...
    -- Change row ops. WRITE covers filesystem flushes where the path may be
    -- new-on-disk or an overwrite — push.lua records both cases under the
    -- same op because the FS flush layer doesn't distinguish create from
    -- replace. Earlier code referenced OPS.WRITE expecting it to exist;
    -- without this entry the journal validator rejects fs flush rows with
    -- op=nil and integrate cycle 2 fails after a bounce that re-stages
    -- the same file paths.
    OPS = {
        CREATE = "create",
        UPDATE = "update",
    ...
    FS = {
        ...
        PROJECT_VOLUME = "keeper.components:project_fs",
    },
