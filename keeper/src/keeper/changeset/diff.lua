...
-- ============================================================================
-- Filesystem side: DB content vs project filesystem frontend roots
-- ============================================================================

local function append_manifest(dst, manifest)
    for _, item in ipairs(manifest or {}) do
        table.insert(dst, item)
    end
end

local function snapshot_frontend_roots(view)
    local manifest = {}

    local fe_snap, fe_err = fs_hash.snapshot(view.baseline_fs, "frontend", {
        skip_dirs = consts.FE_SKIP_DIRS,
    })
    if fe_err then return nil, fe_err end
    append_manifest(manifest, fe_snap.manifest)

    if view.baseline_fs:exists("plugins") and view.baseline_fs:isdir("plugins") then
        for entry in view.baseline_fs:readdir("plugins") do
            if entry.type == "directory" and entry.name:sub(1, 1) ~= "." then
                local root = "plugins/" .. entry.name .. "/frontend"
                if view.baseline_fs:exists(root) and view.baseline_fs:isdir(root) then
                    local snap, serr = fs_hash.snapshot(view.baseline_fs, root, {
                        skip_dirs = consts.FE_SKIP_DIRS,
                    })
                    if serr then return nil, serr end
                    append_manifest(manifest, snap.manifest)
                end
            end
        end
    end
...
function M.filesystem_diff(changeset_id, view)
    ...
    -- Walk only frontend roots for baseline (skip build output). This keeps
    -- local-module frontend files comparable without hashing registry source.
    local fe_snap, fe_err = snapshot_frontend_roots(view)
    if fe_err then return nil, fe_err end
