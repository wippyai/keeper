...
local function define_tests()
    describe("record_changes helpers", function()
        describe("validate_rows", function()
            ...
            it("accepts all four ops including write (fs flush)", function()
                -- Regression: integrate cycle 2 of v19 failed with
                -- "rows[1].op invalid: nil" because push.lua passed
                -- CS_OPS.WRITE which wasn't defined in OPS, leaving op=nil.
                -- WRITE is the canonical op for filesystem flushes.
                ...
                test.is_nil(record_changes.validate_rows({
                    valid_row({ category = "filesystem", op = "write",
                                target = "frontend/foo.vue", source = "fs_flushed",
                                status = "applied" }) }),
                    "filesystem flush rows must validate")
            end)
