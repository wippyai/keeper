...
local function check_permission(permission, resource)
    local checked_resource = resource or ""
    local can, err = security.can(permission, checked_resource)
    if err then
        log:warn("Security check error", { permission = permission, error = err })
        return false, "Security check error: " .. err
    end
    if not can then
        log:warn("Permission denied", { permission = permission, resource = checked_resource })
...
local function send_and_wait(message: GovMessage, timeout: string?)
    ...
    local ok = process.send(tostring(consts.PROCESS_NAME), tostring(consts.TOPICS.COMMANDS), message)
    if not ok then
...
local function build_message(operation: string, extra: {[string]: unknown}?): GovMessage
    local message: GovMessage = {
        id        = generate_id(),
        operation = operation,
    ...
    if extra then
        if extra.changeset ~= nil then message.changeset = extra.changeset end
        if extra.version_id ~= nil then message.version_id = tostring(extra.version_id) end
    end
    return message
end

local function dispatch(spec: DispatchSpec)
    local permission = tostring(spec.permission or "")
    local resource = spec.resource and tostring(spec.resource) or nil
    local ok, err = check_permission(permission, resource)
...
function client.get_state(options, timeout)
    local safe_timeout = optional_string(timeout)
