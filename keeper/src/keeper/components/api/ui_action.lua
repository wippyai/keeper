...
type UiArgs = {[string]: unknown}
type DispatchFn = (UiArgs) -> unknown
...
local function handler()
    ...
    local raw_body = req:body_json()
    local body: {[string]: unknown} = type(raw_body) == "table" and (raw_body :: {[string]: unknown}) or {}
    local op = body.op
    ...
    local call_args = args :: UiArgs

    local dispatch = DISPATCH :: {[string]: DispatchFn}
    ...
    if op == "open" and not call_args.auth_token then
        call_args.auth_token = (function()
