...
-- Resolves the project root from the runtime cwd so nothing is hardcoded, then
-- shells out to `docker run node:20-alpine` with the component's directory as
-- the working directory. Stdout/stderr are drained on concurrent coroutines
-- into keeper_fe_build_lines.
...
local function run(args)
    ...
    if project_root == "" then
        local msg = "unable to resolve project root from system.process.cwd()"
