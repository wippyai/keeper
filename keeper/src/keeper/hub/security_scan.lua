...
local function strip_json_fence(text: string): string
    local value = trim(text)
    if string.sub(value, 1, 3) ~= "```" then return value end

    local body = value:match("^```[%w_-]*%s*(.-)%s*```%s*$")
    return trim(body or value)
end

local function decode_review_response(text: unknown)
    local value = strip_json_fence(tostring(text or ""))
    local decoded, decode_err = decode(value)
    if decoded ~= nil then return decoded, decode_err end
...
local function raw_text(value: unknown): string
...
local function entry_source(entry: unknown): string
    ...
    local source = raw_text(entry.source)
    if source ~= "" then return source end
    source = raw_text(data.source)
    if source ~= "" then return source end
    source = raw_text(entry.content)
    if source ~= "" then return source end
...
function Scanner:review_module(node: ScanNode, artifact: ScanArtifact)
    ...
    for _, chunk in ipairs(chunks) do
        ...
        local decoded, decode_err = decode_review_response(resp.result)
