...
local function get_database_types()
    ...
    db_reader = db_reader:with_kind_list({"db.sql.sqlite", "db.sql.postgres", "db.sql.mysql"})
...
local function handler()
    ...
    for _, migration in ipairs(migrations) do
        ...
        if type(target_db) ~= "string" or target_db == "" then
        ...
        local status = check_migration_status(target_db, migration_id)
