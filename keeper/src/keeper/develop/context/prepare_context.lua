...
local function handler(params)
    ...
    -- Inherit the task scope so every nested agent's tool calls are
    -- attributed to the parent task. Without this, internal agents
    -- (context_router, context-chain template) run with arena.context = nil
    -- and audit silently drops their tool calls.
    local task_id = ctx.get("task_id")
    if task_id == "" then task_id = nil end
    local phase = ctx.get("phase")
    if phase == "" then phase = nil end
    local agent_scope = task_id and scope.for_phase(task_id, phase) or nil
