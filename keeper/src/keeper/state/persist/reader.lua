...
type StateReader = {
    _branches: {[integer]: string},
    _entry_ids: {[integer]: string}?,
    _kinds: {[integer]: string}?,
    _namespaces: {[integer]: string}?,
    _attribute_filters: {[string]: unknown}?,
    _deleted: boolean?,
    _limit: integer?,
    _include_chunks: boolean?,
    _include_attributes: boolean?,
    _search_query: string?,
    _source_ids: {[integer]: string}?,
    _target_ids: {[integer]: string}?,
    _edge_types: {[integer]: string}?,
    _mode: string,
    _copy: (StateReader) -> StateReader,
    with_entries: (StateReader, ...string) -> StateReader,
    with_entry_list: (StateReader, {[integer]: string}) -> StateReader,
    with_kinds: (StateReader, ...string) -> StateReader,
    with_kind_list: (StateReader, {[integer]: string}) -> StateReader,
    with_namespaces: (StateReader, ...string) -> StateReader,
    with_attributes: (StateReader, {[string]: unknown}) -> StateReader,
    with_search: (StateReader, string) -> StateReader,
    with_sources: (StateReader, ...string) -> StateReader,
    with_targets: (StateReader, ...string) -> StateReader,
    with_edge_types: (StateReader, ...string) -> StateReader,
    include_deleted: (StateReader) -> StateReader,
    include_chunks: (StateReader) -> StateReader,
    include_attributes: (StateReader) -> StateReader,
    limit: (StateReader, integer) -> StateReader,
    all: (StateReader) -> ({[integer]: {[string]: unknown}}?, string?),
    one: (StateReader) -> ({[string]: unknown}?, string?),
    count: (StateReader) -> (integer, string?),
    exists: (StateReader) -> (boolean, string?),
}
...
function methods:with_entry_list(ids: {[integer]: string})
    if self._mode ~= "entries" then
        return self
    end

    local new_instance = self:_copy()
    new_instance._entry_ids = ids
    return new_instance
end
...
function methods:with_kind_list(kinds: {[integer]: string})
    if self._mode ~= "entries" then
        return self
    end

    local new_instance = self:_copy()
    new_instance._kinds = kinds
    return new_instance
end
...
function methods:all()
    ...
    elseif self._mode == "edges" then
        ...
        for _, edge in pairs(merged) do
            ...
            local metadata_json: string? = nil
            if type(metadata) == "string" then metadata_json = metadata end
            edge.metadata = parse_json_metadata(metadata_json)
