...
local function get_file_config(entry)
    ...
    if type(config) == "table" and entry.meta and type(entry.meta.type) == "string" then
        local by_type = config :: {[string]: FileConfig}
        local type_config = by_type[entry.meta.type]
...
function materialize.get_file_config(entry: RegistryEntry): FileConfig?
    local extension, source_field = get_file_config(entry)
    if not extension and not source_field then return nil end
    return { extension = extension, source_field = source_field }
end

local function build_yaml_entry(entry: RegistryEntry)
...
function materialize.entry(raw_entry: unknown)
    if type(raw_entry) ~= "table" then
        return nil, "Invalid entry: missing id or kind"
    end
    local raw = raw_entry :: {[string]: unknown}
    if type(raw.id) ~= "string" or type(raw.kind) ~= "string" then
        return nil, "Invalid entry: missing id or kind"
    end
    local entry: RegistryEntry = {
        id = raw.id,
        kind = raw.kind,
        meta = type(raw.meta) == "table" and (raw.meta :: {[string]: unknown}) or nil,
        data = type(raw.data) == "table" and (raw.data :: {[string]: unknown}) or nil,
    }
...
function materialize.state_entry_to_registry(state_entry)
    ...
    local entry_id = type(state_entry.id) == "string" and state_entry.id or ""
    local entry_kind = type(state_entry.kind) == "string" and state_entry.kind or ""
    ...
    if source_content and source_content ~= "" then
        ...
        if source_field then
            local field_value = registry_entry.data and registry_entry.data[source_field]
            if type(field_value) == "string" then
                local filename = field_value:match("^" .. consts.RFS.PATH.FILE_PROTOCOL .. "(.+)$")
                if filename then
