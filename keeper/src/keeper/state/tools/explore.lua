...
local function namespace_operation(params: ExploreParams)
    local namespace = params.name
    if not namespace or namespace == "" then
        namespace = params.namespace
    end
    if not namespace or namespace == "" then
        return nil, "namespace is required for namespace operation (pass as 'namespace' or 'name')"
    end
...
local function tree_operation(params: ExploreParams)
    ...
    if type(params.attributes) == "table" then
        for k, v in pairs(params.attributes) do
            table.insert(filter_parts, tostring(k) .. "=" .. tostring(v))
        end
    end
...
local function entries_operation(params: ExploreParams)
...
local function normalize_fts_query(query)
    if not query or query == "" then return query end
    if query:find('"', 1, true) then return query end
    local normalized = query:gsub("[/:%-]", " "):gsub("%s+", " ")
    return (normalized:gsub("^%s+", ""):gsub("%s+$", ""))
end

local function search_operation(params: ExploreParams)
    local branches = get_active_branch_chain(params.branch)
    ...
    if params.query and params.query ~= "" then
        local q = normalize_fts_query(params.query)
        if q and q ~= "" then
            reader = reader:with_search(q)
        end
    end
    ...
    table.insert(lines, "Search: " .. query_display .. " (" .. #unique .. " results" .. filter_display .. ")")
    table.insert(lines, "")
...
local function graph_operation(params: ExploreParams)
    ...
    local function query_edges(entry_id: string, dir: string)
...
local function do_handler(params: ExploreParams)
    ...
    if params.operation == "tree" then
        result, err = tree_operation(params)
    elseif params.operation == "namespace" then
        result, err = namespace_operation(params)
    elseif params.operation == "entries" then
        result, err = entries_operation(params)
    elseif params.operation == "search" then
        result, err = search_operation(params)
    elseif params.operation == "graph" then
        result, err = graph_operation(params)
    end

    if not err and result and type(result) == "string" and params.full ~= true then
        local goal = params.goal
        if not goal or goal == "" then
            if params.operation == "search" and params.query then
                goal = "Entries relevant to: " .. params.query
            elseif params.operation == "entries" and params.ids then
                goal = "Purpose and shape of: " .. table.concat(params.ids, ", ")
            elseif params.operation == "namespace" and params.name then
                goal = "Structure of namespace: " .. params.name
            elseif params.operation == "tree" then
                goal = "High-level map of the registry"
            elseif params.operation == "graph" and params.id then
                goal = "Dependency neighborhood of " .. params.id
            end
        end
        local compressed, _sum_err, was_summarized = summarize.summarize(result, goal, {
            tool = "explore_state:" .. params.operation,
        })
...
local function summarise_explore(params, result, err)
    if err then return "explore failed: " .. tostring(err) end
    local op = params.operation or "?"
    if op == "tree" then
        return "tree from " .. (params.root or "root")
    elseif op == "namespace" then
        return "namespace " .. (params.name or "?")
    elseif op == "entries" then
        return "loaded " .. (params.ids and #params.ids or 0) .. " entries"
    elseif op == "search" then
        local count = type(result) == "table" and (result.total or result.count or 0) or 0
        return "search '" .. (params.query or "?") .. "': " .. count .. " results"
    elseif op == "graph" then
        return "graph for " .. (params.id or "?")
    end
    return op
end
...
local function handler(params)
    ...
    return audit.wrap({
        tool          = "explore",
        discriminator = "explore." .. (params.operation or "?"),
        target        = params.name or params.query or params.id or params.root,
        params        = {
        ...
        summarise = function(result, err) return summarise_explore(params, result, err) end,
    }, function()
