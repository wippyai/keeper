...
-- When the caller compares against a workspace branch (`ws/<changeset_id>`),
-- the changeset can also carry filesystem-only edits that don't show up as
-- registry entries. Surface those alongside the registry diff so agents
-- don't read "0 changes" and conclude the workspace is empty when they
-- actually staged Vue/TS/router edits.
local function fs_changes_for_branch(branch)
    if type(branch) ~= "string" or branch:sub(1, 3) ~= "ws/" then
        return nil
    end
    local changeset_id = branch:sub(4)
    if changeset_id == "" then return nil end
    local rows, err = changeset_repo.list_changes_for_changeset(changeset_id,
        { category = "filesystem" })
    if err or not rows or #rows == 0 then return nil end
    return rows, changeset_id
end
...
local function kind_label(entry)
    ...
    return tostring(entry.kind or "unknown")
end

local function load_branch_entries(branches: {[integer]: string}, namespace_filter: string?, entry_ids: {[integer]: string}?)
    ...
    if entry_ids and #entry_ids > 0 then
        reader = reader:with_entry_list(entry_ids)
    end
...
local function generate_unified_diff(old_text: string, new_text: string, context_lines: number?)
    ...
    local old_lines: {[integer]: string} = {}
    for line in (old_text .. "\n"):gmatch("([^\n]*)\n") do
        table.insert(old_lines, line)
    end

    local new_lines: {[integer]: string} = {}
...
local function summary_mode(params: CompareParams)
    ...
    local source_branches = branch_chain(source_branch)
    local target_branches = branch_chain(target_branch)
    ...
    if #changes.added > 0 then
        ...
        for _, id in ipairs(changes.added) do
            ...
            table.insert(lines, id .. " (" .. kind_label(entry) .. ")")
        end
        table.insert(lines, "")
    end
    ...
    -- Filesystem-only edits (Vue/TS pages, router files, raw fs writes)
    -- live in keeper_changeset_changes, NOT keeper_overlay_entries. Without
    -- this section the comparison reads "0 changes" even when the workspace
    -- carries pages + router edits — which is what tripped agents into
    -- thinking their staged work was already on main.
    local fs_rows, target_changeset_id = fs_changes_for_branch(target_branch)
    if fs_rows then
        table.insert(lines, "=== FILESYSTEM CHANGES (" .. #fs_rows .. ") ===")
        table.insert(lines, "(staged on changeset " .. tostring(target_changeset_id)
            .. "; not part of registry diff above)")
        for _, row in ipairs(fs_rows) do
            table.insert(lines, "  " .. tostring(row.op) .. " " .. tostring(row.target))
        end
        table.insert(lines, "")
    end

    if #changes.added == 0 and #changes.deleted == 0 and #changes.modified == 0
        and not fs_rows then
        table.insert(lines, "No changes detected")
    elseif #changes.added == 0 and #changes.deleted == 0 and #changes.modified == 0
        and fs_rows then
        table.insert(lines, "No registry changes; filesystem changes listed above.")
    end

    return table.concat(lines, "\n")
end

local function tree_mode(params: CompareParams)
    local source_branch = params.source or "main"
    local target_branch = params.target
    local namespace_filter = params.namespace
    ...
    local changes = classify_changes(source_map, target_map)
    local fs_rows, target_changeset_id
    if not namespace_filter then
        fs_rows, target_changeset_id = fs_changes_for_branch(target_branch)
    end
    ...
    table.insert(lines, "  Modified: " .. #changes.modified)
    if fs_rows then
        table.insert(lines, "  Filesystem: " .. #fs_rows)
    end
    table.insert(lines, "")
    ...
    if fs_rows then
        ...
        table.insert(lines, "Filesystem Changes:")
        table.insert(lines, "  changeset: " .. tostring(target_changeset_id))
        for _, row in ipairs(fs_rows) do
            table.insert(lines, "  " .. tostring(row.op) .. " " .. tostring(row.target))
        end
    end
...
local function entries_mode(params: CompareParams)
    local source_branch = params.source or "main"
    local target_branch = params.target
    local namespace_filter = params.namespace
    ...
    local changes = classify_changes(source_map, target_map)
    local fs_rows, target_changeset_id
    if not namespace_filter and (not entry_ids or #entry_ids == 0) then
        fs_rows, target_changeset_id = fs_changes_for_branch(target_branch)
    end
    ...
    if #changes.added > 0 then
        ...
        for _, id in ipairs(changes.added) do
            local entry = target_map[id]
            table.insert(lines, "Entry: " .. id .. " (" .. kind_label(entry) .. ")")
            table.insert(lines, "")
    ...
    if #changes.modified > 0 then
        ...
        for _, id in ipairs(changes.modified) do
            ...
            table.insert(lines, "Entry: " .. id .. " (" .. kind_label(target_entry) .. ")")
            table.insert(lines, "")
    ...
    if fs_rows then
        ...
        table.insert(lines, "")
        table.insert(lines, "(staged on changeset " .. tostring(target_changeset_id)
            .. "; not part of registry entry diff above)")
        table.insert(lines, "")
        for _, row in ipairs(fs_rows) do
            table.insert(lines, "  " .. tostring(row.op) .. " " .. tostring(row.target))
        end
