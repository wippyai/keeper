    ...
    describe("Agent Tools", function()
        ...
        describe("explore", function()
            ...
            it("tree operation with root prefix filters namespaces", function()
                local result, err = call({ operation = "tree", branch = "main", root = "keeper.state.tools" })
                test.is_nil(err)
                test.not_nil(result)
                test.eq(type(result), "string")
            end)
        ...
        describe("compare", function()
            local function create_fs_only_changeset(title, target_path)
                local cs_id = "compare-fs-" .. uuid.v7()
                local ws, cerr = changeset_repo.create_changeset({
                    changeset_id     = cs_id,
                    title            = title,
                    kind             = changeset_consts.KINDS.SESSION,
                    actor_id         = "test.state_tools",
                ...
                local _, rerr = changeset_repo.record_change({
                    changeset_id = cs_id,
                    category     = changeset_consts.CATEGORIES.FILESYSTEM,
                    op           = changeset_consts.OPS.UPDATE,
                    target       = target_path,
                    source       = "test",
            ...
            it("tree mode surfaces filesystem-only workspace changes", function()
                local target_path = "frontend/applications/keeper/src/pages/probe.vue"
                local cs_id, ws = create_fs_only_changeset("compare fs tree test", target_path)

                local result, err = compare.handler({
                    mode   = "tree",
                    source = "main",
                    target = ws.state_branch,
                    full   = true,
                })
                test.is_nil(err)
                test.not_nil(result)
                test.is_true(result:find("Filesystem: 1", 1, true) ~= nil,
                    "tree compare must not hide fs-only staged work")
                test.is_true(result:find("Filesystem Changes:", 1, true) ~= nil)
                test.is_true(result:find(target_path, 1, true) ~= nil)
                test.is_true(result:find("No changes detected", 1, true) == nil,
                    "fs-only branches are still changed branches")

                changeset_repo.drop_changeset(cs_id, ws.state_branch)
            end)

            it("entries mode surfaces filesystem-only workspace changes", function()
                local target_path = "frontend/applications/keeper/src/pages/probe-entry.vue"
                local cs_id, ws = create_fs_only_changeset("compare fs entries test", target_path)
                ...
                test.is_nil(err)
                test.not_nil(result)
                test.is_true(result:find("=== FILESYSTEM CHANGES (1) ===", 1, true) ~= nil,
                    "entries compare must not hide fs-only staged work")
                test.is_true(result:find(target_path, 1, true) ~= nil)
                test.is_true(result:find("No registry changes; filesystem changes listed above.", 1, true) ~= nil)
                test.is_true(result:find("No changes detected", 1, true) == nil,
                    "fs-only entries compare must not claim the branch is empty")
        ...
        describe("apply", function()
            it("uses explicit branch when applying patches without ctx branch", function()
                local suffix = uuid.v4():sub(1, 8)
                local entry_name = "entry_" .. suffix
                local entry_id = "app.apply_test:" .. entry_name
                local branch_name = "test-apply-" .. suffix
                local definition = [[
version: "1.0"
namespace: app.apply_test

entries:
  - name: ]] .. entry_name .. [[
    kind: registry.entry
    meta:
      comment: apply explicit branch regression
]]

                local result, err = apply.handler({
                    branch = branch_name,
                    title = "apply explicit branch regression",
                    patches = {{
                        target = "entry",
                        op = "set",
                        id = entry_id,
                ...
                test.eq(result.stage, "apply")
                test.eq(result.state_branch, branch_name)
        ...
        describe("reset", function()
            it("requires entry_id parameter", function()
                local result, err = reset.handler({})
                test.is_nil(result)
                test.not_nil(err)
            end)

            it("rejects empty entry_id", function()
                local result, err = reset.handler({ entry_id = "" })
                test.is_nil(result)
                test.not_nil(err)
            end)
        end)
