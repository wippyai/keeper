...
function M.mark(task_id, params)
    ...
    if not step then
        -- Soft-fail: orchestrators occasionally hallucinate step ids that
        -- aren't in the plan. Returning a warning string instead of a tool
        -- error keeps the trail tidy (no red integrate_stage / tool_call
        -- error_message) while telling the agent what it did wrong so it
        -- can stop inventing steps.
        return "step_done: no-op — " .. tostring(err) ..
            ". Only step_ids that exist in the active plan (see read_context) are valid."
    end
