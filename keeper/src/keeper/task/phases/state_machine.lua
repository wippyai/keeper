...
-- Bounce-cap terminal semantics:
--   P.FINISH    → close_task marks task_status='completed' (work landed on main)
--   P.ABANDONED → close_task marks task_status='abandoned'  (work did NOT land — feasibility fail)
--   "ask_user"  → flow pauses, task_status='waiting_for_user' (give human a chance)
-- Cap exhaustion never means "shipped" — agents gave up retrying. The
-- right move depends on WHY:
--   * Convergence ping-pong (review-bugs / implement-pushes / test-bugs /
--     integrate-fails) — the human can usually break the loop with a
--     pointer or a clarification → ask_user. Don't throw the work away.
--   * Spec infeasibility (implement keeps emitting spec_wrong) — the
--     agents are telling us the design itself is wrong → abandoned.
M.BOUNCE_CAPS = {
    [P.REVIEW] = {
        [P.IMPLEMENT] = { cap = 1, terminal = "ask_user",
            note = "Review returned bugs once and implement tried to fix. If review is still red on the next bounce, pause for the user — they can adjudicate (the bug may be a reviewer false positive)." },
    },
    [P.IMPLEMENT] = {
        [P.DESIGN] = { cap = 3, terminal = P.ABANDONED,
            note = "Implement keeps rejecting the spec. The agents are saying the design is infeasible — abandon for a redesign rather than ask the user to override." },
        [P.REVIEW] = { cap = 5, terminal = "ask_user",
            note = "Implement has pushed repeatedly without closure. Pause for the user — they may need to relax acceptance or give a hint." },
    },
    [P.TEST] = {
        [P.IMPLEMENT] = { cap = 3, terminal = "ask_user",
            note = "Test has bounced to implement repeatedly without the endpoints going green. Pause for the user — runtime debugging often needs a human eye." },
    },
    -- Integrate can hammer implement indefinitely if handlers keep failing
    -- (bad migration, buggy agent-generated test, broken endpoint). Cap at 2
    -- and pause for the user instead of a terminal force-finish — the user
    -- can respond with guidance and the task resumes at implement with the
    -- failure context preserved in phase_summary_integrate.
    [P.INTEGRATE] = {
        [P.IMPLEMENT] = { cap = 3, terminal = "ask_user",
            note = "Integrate has failed and bounced to implement three times. On the next fail the task pauses and asks the user — respond with guidance and implement will resume with the last failure visible in its context." },
    },
}
