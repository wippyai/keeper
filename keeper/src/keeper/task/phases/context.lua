...
local function design_prompt(task, cs, opts, prior)
    ...
    blank(parts)
    append(parts, "## Workflow")
    append(parts, "1. read_context for prior research and use the preloaded context_chain findings.")
    append(parts, "2. Delegate lookup gaps to research with a focused question. You do not have")
    append(parts, "   direct registry / KB / docs lookup tools in this phase.")
    append(parts, "3. Save every concrete finding with save_context")
    append(parts, "4. write_spec with every entry ID, kind, and dependency")
