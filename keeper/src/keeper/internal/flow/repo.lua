...
type NodeReader = {
    all: (NodeReader) -> ({[integer]: FlowNode}?, string?),
    count_by_status: (NodeReader) -> ({[integer]: {[string]: unknown}}?, string?),
    with_nodes: (NodeReader, string) -> NodeReader,
    one: (NodeReader) -> (FlowNode?, string?),
}
...
function M.get_flow(dataflow_id: string): (Flow?, string?)
...
function M.all_nodes(dataflow_id: string)
    local reader = node_reader.with_dataflow(dataflow_id) :: NodeReader
    local nodes, err = reader:all()
    if err then error("all_nodes: " .. err) end
    return nodes or {}
end

-- Status counts for a flow's nodes.
function M.status_counts(dataflow_id: string)
    local reader = node_reader.with_dataflow(dataflow_id) :: NodeReader
    local counts, err = reader:count_by_status()
    if err then error("status_counts: " .. err) end
    return counts or {}
end

-- Get one node.
function M.get_node(dataflow_id: string, node_id: string)
    local reader = node_reader.with_dataflow(dataflow_id) :: NodeReader
    local n, err = reader:with_nodes(node_id):one()
    if err then error("get_node: " .. err) end
    return n
end

-- All data rows scoped to a node, ordered by created_at.
function M.node_data(dataflow_id: unknown, node_id: unknown, opts: DataReadOptions?)
    if type(dataflow_id) ~= "string" or dataflow_id == "" then
        error("node_data: dataflow_id must be a string")
    end
    if type(node_id) ~= "string" or node_id == "" then
        error("node_data: node_id must be a string")
    end
    opts = opts or {}
...
function M.flow_data(dataflow_id: unknown, opts: DataReadOptions?)
    if type(dataflow_id) ~= "string" or dataflow_id == "" then
        error("flow_data: dataflow_id must be a string")
    end
    opts = opts or {}
    local r = data_reader.with_dataflow(dataflow_id) :: DataReader
    if opts.types then r = r:with_data_types(opts.types) end
...
function M.adjacency(nodes)
    local by_id: {[string]: FlowNode} = {}
    local children: {[string]: {[integer]: string}} = {}
    local roots: {[integer]: string} = {}
