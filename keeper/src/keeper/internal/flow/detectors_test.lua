...
local function define_tests()
    describe("Flow detectors", function()
        local function must_finding(findings, index)
            local row = findings[index]
            if not row then error("finding missing at index " .. tostring(index)) end
            return row
        end
        ...
        describe("detect_iteration_exhaustion", function()
            it("flags agent node with MAX_ITERATIONS error code", function()
                ...
                test.eq(#findings, 1)
                local finding = must_finding(findings, 1)
                test.eq(finding.type, "iteration_exhaustion")
                test.eq(finding.severity, "high")
                test.eq(finding.node_id, "a1")
            end)
            ...
            it("flags agent node by status_message substring", function()
                ...
                test.eq(#findings, 1)
                test.is_true(must_finding(findings, 1).evidence:find("Maximum iterations") ~= nil)
            end)
        ...
        describe("detect_cycle_exhaustion", function()
            it("flags cycle node with MAX_ITERATIONS_EXCEEDED", function()
                ...
                test.eq(#findings, 1)
                local finding = must_finding(findings, 1)
                test.eq(finding.type, "cycle_exhaustion")
                test.eq(finding.node_id, "c1")
            end)
        ...
        describe("detect_tool_failures", function()
            it("flags each failed tool.call node", function()
                ...
                test.eq(#findings, 1)
                local finding = must_finding(findings, 1)
                test.eq(finding.type, "tool_failure")
                test.eq(finding.severity, "medium")
                test.eq(finding.node_id, "t1")
                test.eq(finding.parent_id, "a1")
                test.is_true(finding.title:find("boom") ~= nil)
            end)
            ...
            it("falls back to metadata.error.message when error_message is absent", function()
                ...
                test.eq(#findings, 1)
                test.is_true(must_finding(findings, 1).evidence:find("nested") ~= nil)
            end)
        end)
        ...
        describe("detect_retry_loops", function()
            it("flags a tool called 3+ times with identical args", function()
                ...
                test.eq(#findings, 1)
                local finding = must_finding(findings, 1)
                test.eq(finding.type, "retry_loop")
                test.eq(finding.node_id, "a1")
                test.is_true(finding.title:find("search called 3x") ~= nil)
            end)
        ...
        describe("detect_sequence_violations", function()
            it("flags observations carrying known precondition errors", function()
                ...
                test.eq(#findings, 1)
                local finding = must_finding(findings, 1)
                test.eq(finding.type, "sequence_violation")
                test.is_true(finding.title:find("Cannot modify main branch") ~= nil)
                test.is_true(finding.suggested:find("set_branch") ~= nil)
            end)
            ...
            it("searches encoded JSON body when value is a table", function()
                ...
                test.eq(#findings, 1)
                test.is_true(must_finding(findings, 1).title:find("not found") ~= nil)
            end)
        end)
        ...
        describe("analyze", function()
            ...
            it("sorts high severity before medium, then by type", function()
                ...
                test.eq(#out, 3)
                local first = must_finding(out, 1)
                local second = must_finding(out, 2)
                local third = must_finding(out, 3)
                test.eq(first.severity, "high")
                test.eq(second.severity, "high")
                test.eq(third.severity, "medium")
                test.eq(first.type, "cycle_exhaustion",
                    "ties within high severity sort by type ascending")
                test.eq(second.type, "iteration_exhaustion")
            end)
        end)
    end)
end
