...
function M.detect_retry_loops(data_rows, findings)
    local actions_by_node: {[string]: {[integer]: unknown}} = {}
    for _, row in ipairs(data_rows) do
        if row.type == "agent.action" and type(row.node_id) == "string" then
            local rows = actions_by_node[row.node_id]
            if not rows then
                rows = {}
                actions_by_node[row.node_id] = rows
            end
    ...
    for node_id, actions in pairs(actions_by_node) do
        ...
        local counts: {[string]: {count: integer, tool: string, args: unknown}} = {}
        for _, f in ipairs(flat) do
            local h = args_hash(f.name, f.args)
            counts[h] = counts[h] or { count = 0, tool = f.name, args = f.args }
            local count_row = counts[h]
            count_row.count = (count_row.count or 0) + 1
        end
