...
local function add_column_if_not_exists(db, ddl, label)
    local _, err = db:execute(ddl)
    if err then error(label .. ": " .. tostring(err)) end
end
        ...
        database("postgres", function()
            up(function(db)
                add_column_if_not_exists(db,
                    "ALTER TABLE keeper_mcp_tokens ADD COLUMN IF NOT EXISTS issued_by TEXT",
                    "add issued_by")
                add_column_if_not_exists(db,
                    "ALTER TABLE keeper_mcp_tokens ADD COLUMN IF NOT EXISTS revoked_at INTEGER",
                    "add revoked_at")
                add_column_if_not_exists(db,
                    "ALTER TABLE keeper_mcp_tokens ADD COLUMN IF NOT EXISTS revoked_by TEXT",
                    "add revoked_by")
