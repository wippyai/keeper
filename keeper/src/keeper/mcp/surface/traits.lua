...
local filter_matches = mcp_authorize.filter_matches
...
function M.tool_allowed(session, tool_id)
    ...
    local tags = entry.meta.tags or {}
    if filter ~= nil and not filter_matches(filter, tool_id, tags) then
        return false, "tool not allowed by session filter: " .. tostring(tool_id)
    end
...
local function prune_missing_traits(active)
    local kept = {}
    local dropped = {}
    if type(active) ~= "table" then return kept, dropped end
...
function M._prune_missing_traits(active)
...
function M.resolve(session)
    ...
    if #dropped > 0 and supports_traits(session) and type(token) == "string" then
    ...
    local tools = {}
    for name, tool in pairs(compiled.tools or {}) do
        local registry_id = tool and tool.registry_id
        if registry_id then
            local allowed = M.tool_allowed(session, registry_id)
            if allowed then tools[name] = tool end
        end
    end
...
function M.resolve_tools_only(session)
    ...
    for _, e in ipairs(entries) do
        local tags = (e.meta and e.meta.tags) or {}
        local allowed = filter_matches(filter, e.id, tags)
        if allowed then
            allowed = M.tool_allowed(session, e.id)
        end
        if allowed then
