...
import { HOST_API, AXIOS_INSTANCE, WIPPY_INSTANCE, WIPPY_CONFIG, ON_SUBSCRIPTION } from '../constants'
...
export function useOn(): OnSubscription {
  const on = inject(ON_SUBSCRIPTION, null)
  if (on) return on
  return useWippy().on as unknown as OnSubscription
}
